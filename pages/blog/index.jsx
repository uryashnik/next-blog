...
const Blog = ({posts, categories}) => {
    ...
    return (<div className='mx-auto lg:w-5/6'>
            ...
            <AnimateHeight open={open}>
                <div className='grid grid-cols-3 gap-4 md:grid-cols-4 md:gap-6 pt-6 md:pt-8 lg:pt-12 '>
                    {categories.map((name) => (<label key={name} className='cursor-pointer whitespace-nowrap'>
                            <input
            ...
            <div>
                {content.length ? (content.map((post) => {
                        const tags = Object.keys(post.tags);
                        return (<div className='mb-10' key={post.id}>
            ...
            <Button
                className='w-full'
                onClick={() => {
                    Router.push(`/blog/?page=${router.query.page ? Number(router.query.page) + 1 : 2}`, undefined, {scroll: false});
                }}
            >
                <div className='flex items-center justify-center'>
                    Load
                    <div className='px-2'>
                        <Spinner loading={isLoading} fill={theme === 'dark' ? COLOR_WHITE : COLOR_PRIMARY} />
                    </div>
                    More
                </div>
            </Button>
